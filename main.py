import socket
import requests
import json
import time
import threading

HOST = '192.168.1.1'
PORT = 5000

def transaction_requests(data):
    try:
        url = 'http://localhost:8000/api/transactions'
        requests.post(url, data=json.dumps(data))
    except requests.exceptions.RequestException as e:
        print(f"Error al enviar datos RS232: {e}")

def keep_alive(s):
    """Hilo que envía un 'ping' cada 5 s para mantener la conexión TCP viva."""
    while True:
        try:
            s.sendall(b'PING')
            time.sleep(200)
        except Exception:
            break

def main():
    while True:
        try:
            with socket.create_connection((HOST, PORT), timeout=10) as s:
                print(f"Conectado al RUT956 ({HOST}:{PORT}). Esperando datos...\n")
                threading.Thread(target=keep_alive, args=(s,), daemon=True).start()

                while True:
                    try:
                        data = s.recv(1024)
                        if not data:
                            print("\n Conexión cerrada por el RUT956.")
                            break
                        decoded = data.decode(errors="ignore").strip()
                        if decoded.startswith('$>') and decoded.endswith('#'):
                            data_string = decoded[2:-1]
                            aux_data = str(data_string[1:-1])
                            code = aux_data[25:34]
                            card_type = int(aux_data[14:18])
                            date = f"{aux_data[6:8]}-{aux_data[8:10]}-{aux_data[10:14]}"
                            time_str = f"{aux_data[0:2]}:{aux_data[2:4]}:{aux_data[4:6]}"
                            cost = float(int(aux_data[46:54]) / 100)
                            balance = float(int(aux_data[-8:]) / 100)
                            data_formated = {
                                "card_code": code,
                                "card_type": card_type,
                                "date": date,
                                "time": time_str,
                                "amount": cost,
                                "balance": balance,
                            }
                            transaction_requests(data_formated)
                    except socket.timeout:
                        print("\n Timeout esperando datos.")
                    except Exception as e:
                        print(f"Error recibiendo datos: {e}")
                        break
        except Exception as e:
            print(f"Error de conexión: {e}")
            print("\n Reintentando conexión en 5 s...\n")
            time.sleep(5)

if __name__ == "__main__":
    try:
        main()
    except KeyboardInterrupt:
        print("\nPrograma interrumpido por el usuario (Ctrl+C).")
